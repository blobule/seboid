...
public class ActivityUdeMDetail extends Activity {
	...
	public static final String[] projection = new String[] { DBHelper.C_ID,
			DBHelper.C_TITLE, DBHelper.C_LINK, DBHelper.C_DESC,
			DBHelper.C_TIME, DBHelper.C_LONGDESC, DBHelper.C_IMAGE,
			DBHelper.C_LU, DBHelper.C_CATEGORY };
	...
	protected void onResume() {
		...
		cursor = getContentResolver().query(UdeMContentProvider.CONTENT_URI,
				projection, where, null, DBHelper.C_TIME + " DESC");

		// cursor=db.query(DBHelper.TABLE,projection,where, null, null, null,
		...
		// Toast.makeText(this,"found id="+id+" at pos="+cursor.getPosition(),
	...
	protected void onPause() {
		super.onPause();
		// set id to the current id we are looking at, so we can go back there
		// later...
		id = cursor.getLong(0);
	...
	//
	// String
	// style="<style type=\"text/css\">p { color:#ffffff; } a { color:#8080ff; } h2 { color:#ffffff; }</style>";
	// String titre="<h2>"+title+"</h2>";
	// String
	// lien="<p style=\"text-align:right;\"><a href=\""+link+"\">La suite...</a>";
	...
	//
	// //String
	// meta="<meta name=\"format-detection\" content=\"telephone=yes\" /><meta name=\"format-detection\" content=\"address=no\" />";
	// web.getSettings().setJavaScriptEnabled(false);
	// web.loadDataWithBaseURL(null,makeWebBody(true),
	// "text/html","utf-8",null);
	// // web.setEnabled(true);
	//
	// //Toast.makeText(UdeMRssListActivity.this,
	// htmlData,Toast.LENGTH_LONG).show();
	...
	private class AwesomePagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(View collection, int position) {
			...
			// LinearLayout detail =
			// (LinearLayout)inflater.inflate(R.layout.detail_page_bleed,null);
			LinearLayout detail = (LinearLayout) inflater.inflate(
					R.layout.detail_page, null);
			...
			ImageView image = (ImageView) detail
					.findViewById(R.id.detail_image);
			ProgressBar loading = (ProgressBar) detail
					.findViewById(R.id.detail_loading);
			...
			if (imgURL.equals("")) {
				...
				if (imageCache.containsKey(imgURL)) {
					...
					new DownloadImageTask(image, loading).execute(cursor
							.getString(6));
				}
			}
		...
		public void finishUpdate(ViewGroup vg) {
			...
			if (!lecture[pos].lu) {
				...
				Uri itemUri = Uri.parse(UdeMContentProvider.CONTENT_URI + "/"
						+ lecture[pos].id);
				getContentResolver().update(itemUri, val,
						DBHelper.C_ID + "=" + lecture[pos].id, null);

				// try {
				// db.update(DBHelper.TABLE, val,
				// DBHelper.C_ID+"="+lecture[pos].id , null);
	...
	// / ca pourrait planter si on detruit l'interface View avant que le load
	// soit fini...
	// /
	// / <in,progress,out>
	...
	private class DownloadImageTask extends
		...
		protected void onPreExecute() {
			image.setVisibility(View.INVISIBLE); // pas GONE, comme ca on garde
													// l'espace...
		...
		protected void onPostExecute(Drawable result) {
			if (result != null) {
				...
				// AnimationUtils.loadAnimation(ActivityUdeMDetail.this,
				// R.anim.fadein);
	...
	public void instructions() {
		...
		adb.setMessage("Vous pouvez passer à la prochaine nouvelle en faisant glisser la page vers la gauche (ou vers la droite)...");
	...
	public void addBookmark(String title, String url) {
		...
		final Uri newBookmark = getContentResolver().insert(
				Browser.BOOKMARKS_URI, bookmarkValues);
	}
